...
for file in "${my_files[@]}"; do
   ...
   for i in {1..5}; do
       ...
       start=$(curl -s --trace-time "http://10.152.183.128" -H "Host: streaming-s3.default.svc.cluster.local" --data-binary @storage/$file -H "x-target: func-b")
       curl -s --trace-time "http://10.152.183.128" -H "Host: decode-s3.default.svc.cluster.local" -d "decode" -H "x-filename: decoder-frame-1.jpg"
       end=$(curl -s --trace-time "http://10.152.183.128" -H "Host: recognition-s3.default.svc.cluster.local" -d @storage/$file -H "x-target: func-b" -H "x-filename: image.jpg" -H "x-comm: S3")
