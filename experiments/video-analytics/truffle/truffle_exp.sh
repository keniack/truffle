...
for file in "${my_files[@]}"; do
  total_time=0
   for i in {1..5}; do
       ...
       duration=$(curl -s --trace-time "http://10.152.183.128" -H "Host: streaming.default.svc.cluster.local" --data-binary @storage/$file -H "x-target: func-b" -H "x-filename: video.mp4" -H "x-comm: TRUFFLE")
